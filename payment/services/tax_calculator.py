...
class CampaignTaxCalculator:
    ...
    def calculate_new_tax_data(
            ...
            keep_commission_constant: bool = False
    ) -> Dict:
        """
    ...
    def calculate_diff(self, new_data: Dict) -> Dict:
        ...
        return {
            ...
            'vat_diff': vat_diff,
            'commission_to_pay': max(commission_diff, 0),
            'vat_to_pay': vat_diff,
    ...
    def calculate_replacement_amount(
        ...
        if new_content_cost is None:
            ...
            is_team_replacement = False
        else:
            # حالت جایگزینی تیم محتوا
            new_content_cost = int(new_content_cost)
            new_influencer_cost = current['influencer_cost']
            base_cost = new_content_cost
            is_team_replacement = True
        ...
        # ✅ اصلاح: tax_base باید از new_influencer_cost استفاده کنه نه current['influencer_cost']
        tax_base = new_influencer_cost + new_content_cost + current['commission'] + commission_to_pay
        new_total_vat = int(tax_base * self.VAT_PERCENT)

        old_vat = current['total_vat']
        vat_diff = new_total_vat - old_vat

        total_deduct = base_cost + commission_to_pay + vat_diff
    ...
    def _build_breakdown(self, base_cost: int, commission_diff: int, vat_diff: int, total_deduct: int,
        ...
        # ۱. هزینه پایه
        if base_cost > 0:
            label = 'هزینه تیم جدید' if is_team_replacement else 'هزینه کانال‌های جدید'
            breakdown.append({
                'label': label,
                'amount': base_cost,
        ...
        # ۳. مابه‌التفاوت کمیسیون (فقط اگر مثبت باشه)
